...
Create the first GitHub repository as a private empty repository named
`shorts-blocker-kids`: no README, no `.gitignore`, and no license generated by
GitHub. Add the local repository as the first push source only after the local
secret scan and quality gate pass.
...
Tracked website preview screenshots under `website/preview/` are intentional
visual review evidence. Generated website output under `website/dist/` and
Gradle/Android build outputs remain untracked.
